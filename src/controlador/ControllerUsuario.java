...
public class ControllerUsuario {
    ...
    public void cargarCombobox() {
        try {
            Vector v = new Vector();
            this.vista.getCbxIdPersona().addItem("Seleccione una persona");
 ...
 //llamar
    public void reporteGeneral() {
        Resouces.imprimirReeporte(ManagerFactory.getConnection(manager.getEntityManagerFactory().createEntityManager()), "/reportes/Usuario.jasper",new HashMap());
  }
    ...
    public void reporteIndividual() {
        ...
        Resouces.imprimirReeporte(ManagerFactory.getConnection(manager.getEntityManagerFactory().createEntityManager()), "/reportes/ReporteUsuarioIndividual.jasper",parametros);
  }else{
        Resouces.warning("!Usuario no Seleccionada!", "Seleccione un usuario");
        }
    }
    ...
    public boolean validacionesCamposUsuario() {
        ...
        if (!this.vista.getTxtUsuario().getText().isEmpty()) {
            //segunda  valid
            if (!this.vista.getTxtContrasenia().getText().isEmpty()) {
                //segunda  valid
                if (this.vista.getCbxIdPersona().getSelectedIndex()!=0) {
         
                } else {
                    Resouces.warning("Campos Vacios!!", "Nota: Es necesario Seleccionar una Persona");
                }
            } else {
                Resouces.warning("Campos Vacios!!", "Nota: Es necesario llenar la Contraseña");
            }
        } else {
            Resouces.warning("Campos Vacios!!", "Nota: Es necesario llenar el Nombre de Usuario");
        }
        return validado;
    }
}
