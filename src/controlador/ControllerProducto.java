...
public class ControllerProducto {
 ...
 //llamar
    public void reporteGeneral() {
        Resouces.imprimirReeporte(ManagerFactory.getConnection(manager.getEntityManagerFactory().createEntityManager()), "/reportes/Producto.jasper",new HashMap());
  }
     public void reporteIndividual() {
        if(producto!=null){
        Map parametros = new HashMap<>();
        parametros.put("id",producto.getIdproducto());
        Resouces.imprimirReeporte(ManagerFactory.getConnection(manager.getEntityManagerFactory().createEntityManager()), "/reportes/ReporteProductoIndividual.jasper",parametros);
  }else{
        Resouces.warning("!Producto no Seleccionada!", "Seleccione unm producto");
        }
    }
    public void guardarProducto() {
        if (validacionesCamposProducto() == true) {
    ...
    public boolean validacionesCamposProducto() {
        Validaciones validar = new Validaciones();
        boolean validado = false;
        if (!this.vista.getTxtNombre().getText().isEmpty()) {
            if (validar.ValidarTextoConEspacio(this.vista.getTxtNombre().getText())) {
                //segunda  valid
                if (!this.vista.getTxtPrecio().getText().isEmpty()) {
                    //segunda  valid
                    if (!this.vista.getTxtCantidad().getText().isEmpty()) {

                        //Segunda valid
                    } else {
                        Resouces.warning("Campos Vacios!!", "Nota: Es necesario llenar la Cantidad");
                    }
                } else {
                    Resouces.warning("Campos Vacios!!", "Nota: Es necesario llenar el Precio");
                }
            } else {
                Resouces.warning("Campos Vacios!!", "Nota: Es necesario llenar correctamente el Precio");
            }
        } else {
            Resouces.warning("Campos Vacios!!", "Nota: Es necesario llenar los Nombre");
        }
        return validado;
    }
}
